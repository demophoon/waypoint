...
func newDeployment(c k8sConfig, opts *InstallRunnerOpts) (*appsv1.Deployment, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("could not parse cpu limit resource %s: %s", c.cpuLimit, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("could not parse memory limit resource %s: %s", c.memLimit, err)
	}
